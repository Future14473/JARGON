...
class MotionState1d(val x: Double, val v: Double, val a: Double = 0.0) {
    ...
    /**
     * Returns the motion state, the case with positive velocity, after moving a displacement of [x] relative to the
     * current state.
     *
    ...
    /**
     * Returns the motion state, the case with positive velocity, when this state reaches a displacement of [x] relative
     * to the global frame.
     *
     * This may return a velocity of NaN if this state will never reach a displacement of [x]
     */
    fun atDist(x: Double): MotionState1d =  //may return v of NaN if impossible
        MotionState1d(x, sqrt(v.squared() + 2 * a * (x - this.x)), a)
}
