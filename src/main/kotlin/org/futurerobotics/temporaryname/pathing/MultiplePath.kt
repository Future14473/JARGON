...
sealed class MultipleGeneric<Path : GenericPath<Point>, Point : CurvePoint>(
    ...
    init {
        val paths: MutableList<Path> = ArrayList(if (paths is Collection) paths.size else 10)
        paths.forEach {
            if (it is MultipleGeneric<*, *>) paths += it.paths.filterIsPath()
            else paths += it
        }
        this.paths = paths
    }
    ...
    override fun stepper(): Stepper<Double, Point> {
        var i: Int = -1
        var curStepper: Stepper<Double, Point>? = null
        return object : Stepper<Double, Point> {
...
class MultipleCurve(paths: Iterable<Curve>, checkContinuity: Boolean = true) :
    ...
    override fun checkPointContinuity(prev: CurvePoint, cur: CurvePoint) {
        ...
        checkCont("PositionSecondDeriv", prev.positionSecondDeriv, cur.positionSecondDeriv)
...
class MultiplePath(paths: Iterable<Path>, checkContinuity: Boolean = true) :
    ...
    override fun checkPointContinuity(prev: PathPoint, cur: PathPoint) {
        ...
        checkCont("HeadingDeriv/Curvature", prev.headingDeriv, cur.headingDeriv)
    }
}
